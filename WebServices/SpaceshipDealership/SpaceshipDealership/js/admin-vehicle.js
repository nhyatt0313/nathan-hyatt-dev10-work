    ...
    $.ajax({
        ...
        success: function (data, status) {
            $.each(data, function (index, vehicle) {
                ...
                row += '<tr>';
                row += '<th></th>';
                row += '<td><button type="button" onclick="editVehicle(' + id + ')" class="btn btn-primary">Edit</button></td>';
                row += '</tr>';
                row += '</table>';
                row += '</div>';
                row += '</div>';
...
function editVehicle(id){
    // take to the edeit vehicle page
    var userId = getUrlLogin()["userId"];
    location.assign('edit-vehicle.html?id='+id+'&userId='+userId);
}

function addVehicle(){
    // take to the add vehicle page
    var userId = getUrlLogin()["userId"];
    location.assign('add-vehicle.html?userId='+userId);
}
